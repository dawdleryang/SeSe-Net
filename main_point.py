...
def predict(model_fname, data_dir, data_csv, out_csv, bShowFlag=False):
    ...
    # import pdb; pdb.set_trace()
    arrX1 = np.zeros(len(img_ids))
    arrY1 = np.zeros(len(img_ids))
    arrX2 = np.zeros(len(img_ids))
    arrY2 = np.zeros(len(img_ids))
    
    arrX1 = p[:, 0] - pts1[:, 0]
    arrY1 = p[:, 1] - pts1[:, 1]
    arrX2 = p[:, 2] - pts2[:, 0]
    arrY2 = p[:, 3] - pts2[:, 1]
    print("mean shift: ", np.mean(abs(arrX1)), np.mean(abs(arrY1)), np.mean(abs(arrX2)), np.mean(abs(arrY2)))
    print("min shift: ", np.min(abs(arrX1)), np.min(abs(arrY1)), np.min(abs(arrX2)), np.min(abs(arrY2)))
    print("max shift: ", np.max(abs(arrX1)), np.max(abs(arrY1)), np.max(abs(arrX2)), np.max(abs(arrY2)))
    # import pdb; pdb.set_trace()
    
    for i in range(len(img_ids)):
        pt1 = [p[i, 0], p[i, 1]]
        pt2 = [p[i, 2], p[i, 3]]
        diff1 = abs(arrX1[i]) + abs(arrY1[i])
        diff2 = abs(arrX2[i]) + abs(arrY2[i])
        diff = max(diff1, diff2)
        if bShowFlag:
            show_img_4(imgs[i], pt1, pt2, pts1[i], pts2[i], img_ids[i])




def predict_old(model_fname, data_dir, data_csv, out_csv):
    #import pdb; pdb.set_trace()
    img_ids, imgs, pts1, pts2 = load_data(data_dir, data_csv)
    height, width = imgs[0].shape
    X = prepare_data(imgs)
...
def show_img_4(img, pts1, pts2, gt1, gt2, title='Image'):
    ...
    dif_p2 = (abs(x2 - gx2) + abs(y2 - gy2)) / 2.0  # np.arctan2(y2-y1, x2-x1) * 180/math.pi - np.arctan2(gy2-gy1, gx2-gx1) * 180/math.pi
...
def show_img_4_old(img, pts1, pts2, gt1, gt2, title='Image'):
    import sys
    import matplotlib.pyplot as plt
    #import matplotlib.patches as patches
...
if __name__ == '__main__':
    ...
    predict('./saved_{}.model'.format(i), './data/testing/frames', './data/testing/ch234.csv', 'pred_234.csv', False)
